...
The very first commercial "solid state camera", the [Cromemco Cyclops](https://en.wikipedia.org/wiki/Cromemco_Cyclops), was issued in 1975 equipped with a [32x32 pixels MOS image sensor](/Pictures%20and%20datasheets/Popular%20Electronics-1975-02.pdf) tinkered from a [1024 bits memory chip](http://wsudbrink.dyndns.org:8080/cyclops/index.html). The chip was retrofitted with a transparent window instead of its usual metal protecting lid to take advantage of the adverse effect of light on stored bits. Surprisingly, the concept was not really followed by any active development.

The Charge Coupled Device or [CCD](https://en.wikipedia.org/wiki/Charge-coupled_device), completely different technology developped at about the [same time](https://archive.nytimes.com/lens.blogs.nytimes.com/2015/08/12/kodaks-first-digital-moment/) for image sensors, took anyway the lead of the 80s digital photography niche. CCD sensors were power hungry and expensive but able to produce images with a good signal to noise ratio, making them relevant for high end market like scientific imaging. 

This hasn't stopped tinkerers from [continuing to play](/Pictures%20and%20datasheets/BYTE%20(1983)%20Build%20the%20Micro%20D-Cam%20Solid-State%20Video%20Camera.pdf) with modified ram chips as affordable alternatives to the expensive CCD sensors. Yet, the demand was there. Not any memory chip was however compatible as allowing to capture a usefull image required a particular gates configuration on silicon and very few manufacturers maintained this kind of odd hardware design on purpose.

The Complementary Metal–Oxide–Semiconductor or [CMOS](https://en.wikipedia.org/wiki/Active-pixel_sensor), alternative image sensing technology at least as old as CCD and based on amplified photodiodes, was relegated to the background all these years due to sensors plagued with critical image artifacts, despite being much cheaper. The development of CMOS-based image sensors for viable commercial applications only took off in the early 90s, impulsed by the [NASA Jet Propulsion Laboratory](https://spinoff.nasa.gov/Spinoff2017/cg_1.html), with the aim of attacking the low end market with cheap and very low power sensors. Good to average image quality was considered acceptable for this market. The JPL design natively integrated automatic calibration for each pixel of a sensor.
