...
The Complementary Metal–Oxide–Semiconductor or [CMOS](https://en.wikipedia.org/wiki/Active-pixel_sensor), alternative image sensing technology at least as old as CCD and based on amplified photodiodes, was relegated to the background all these years due to sensors plagued with critical image artifacts, despite being much cheaper due to easy integration to existing clean room processes. The development of CMOS-based image sensors for viable commercial applications only took off in the early 90s, impulsed by the [NASA Jet Propulsion Laboratory](https://spinoff.nasa.gov/Spinoff2017/cg_1.html), with the aim of attacking the low end market with cheap and very low power sensors. Good to average image quality was considered acceptable for this market. The JPL design natively did integrate automatic calibration for each pixel of a sensor (What Mitsubishi one didn't).
