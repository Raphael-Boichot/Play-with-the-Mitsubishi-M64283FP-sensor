...
The very first commercial "solid state camera", the [Cromemco Cyclops](https://en.wikipedia.org/wiki/Cromemco_Cyclops), was issued in 1975 equipped with a [32x32 pixels MOS image sensor](/Pictures%20and%20datasheets/Popular%20Electronics-1975-02.pdf) tinkered from a [1024 bits memory chip](http://wsudbrink.dyndns.org:8080/cyclops/index.html). The chip was retrofitted with a transparent window instead of its usual metal protecting lid to take advantage of the adverse effect of light on stored bits (light discharges the stored bits). Surprisingly, the concept was not really followed by any active development, despite being simple and fun.
