...
CMOS ([Complementary Metal-oxide Semiconductor](https://en.wikipedia.org/wiki/CMOS)) is a quite old technology used in microelectronics to produce logic gates and by extension, memories. By chance, these logic gates are more or less sensitive to visible light. The first commercial digital camera, the [Cromemco Cyclops](https://en.wikipedia.org/wiki/Cromemco_Cyclops) made in 1975, was created by hacking a MOS ram chip with a transparent window. It allows light to reach the silicon surface and progressively flip the bit stored from 1 to 0. By assessing the bits state at different times, a 32x32 image is reconstructed. The development of CMOS-based image sensors for commercial applications however only began in the early 90s, impulsed by the [NASA Jet Propulsion Laboratory](https://spinoff.nasa.gov/Spinoff2017/cg_1.html), with the aim of competiting with the more expensive CCD technology that was the reference at the time.

Mitsubishi company, on its side, has also decided to develop a series of CMOS sensors called “artificial retina” (AR), with the aim of flooding the market with low cost, low power, high performance devices. These sensors were marketed as mimicking the behavior of a biological retina (concept probably inspired by this [American Scientist](https://github.com/Raphael-Boichot/Mitsubishi-M64282FP-dashcam/blob/main/Docs%20and%20research/Bibliography/Mahowald%20(1991)%20The%20Silicon%20Retina.pdf) article from 1991) and be able to take place in domains as broad as safety, health, education, automation, industry and even defense. The first traces of these sensors in Mitsubishi communication [date back to 1994](https://github.com/Raphael-Boichot/Mitsubishi-M64282FP-dashcam/blob/main/Docs%20and%20research/Bibliography/Kyuma%20(1994)%20Artificial%20retinas%20-%20fast%2C%20versatile%20image%20processing.pdf), with planned features like live coupling to neural networks on chip, Fast Fourier Transform, pattern recognition and 1000x1000 pixels resolution at 1000 fps. What I would summarize as a quite ambitious roadmap. We were at this time in the first A.I. hype and "neural networks" in 1994 were the premises of what we call "machine learning" in 2024. You'd better be in to get juicy grants. And they were in.

The commercial boom ot artificial retinas however really began in 1998 with a less ambitious application: the Game Boy Camera, equipped with the first mass-produced unit, the M64282FP sensor. The exact fate of these sensors is then lost in more or less mass-market products (rather less than more in fact), and they have _probably_ equipped Japanese cell phones, consumer multimedia products, fingerprint readers and security cameras. It is almost certain that the M64283FP sensor equipped the [“LaPochee” camera module](https://time-space.kddi.com/ketaizukan/1999/11.html) accompanying [the THZ43 Chiaro cell phone by Mitsubishi](https://time-space.kddi.com/ketaizukan/1999/10.html), which is technically the first mobile phone with (nearly an embedded) camera. Two capture modes were proposed: [1bpp "manga mode" or 4bbp "real mode", 96x96 pixels](https://pc.watch.impress.co.jp/docs/article/990413/tu_ka.htm). According to Mitsubishi, the M64283FP also probably equipped the [PCPICO](https://web.archive.org/web/20020925132513/http://pcpico.com/), a kind of easy-to-use-for-kids-and-elders multimedia interface in which the sensor is probably used as cheap digital camera (the scarse online documentation is all but clear about what the retina was used for, it's not really advertised). Sales figures for the “LaPochee” + THZ43 Chiaro were considered as unsatisfactory. Sales figures for the PCPICO are totally unknown (which probably means they were _very_ unsatisfactory).

Some sensors from that series come in K (industrial, ceramic) as well as FP (home, plastic) packaging. No certified image of the M64283K or specific datasheet exists online (it is just mentioned as possible package in the M64283FP datasheet). Package dimensions are unknown, but probably close or similar to the [M64285K](https://github.com/Raphael-Boichot/Mitsubishi-M64282FP-dashcam/blob/main/Docs%20and%20research/Bibliography/Mitsubishi%20Integrated%20Circuit%20M64285K%20Image%20Sensor.PDF) to ensure hardware compatibility. As industrial products have completely separate and even more obscure markets, it is not excluded that they were common in industry at some point.

The M64283FP sensor probably existed as a [development/evaluation kit](https://nfbcal.org/nfb-rd/1505.html) consisting of a sensor unit comprising lens, the sensor itself and a board with an ADC, an FPGA controller and working on a Windows 95 PC through the ISA bus. A development library in C/C++ was offered with. A [Super Cheap Artificial Retina Evaluation Board](https://github.com/Raphael-Boichot/Mitsubishi-M64282FP-dashcam/blob/main/Docs%20and%20research/Bibliography/MERL%20annual%20report%201999-2000_extract.pdf) was even planned, but probably never sold. I find that this latter design would have been super cool for student projects.

The artificial retinas were able to perform internal real-time image processing with very basic kernels (edge detection, edge enhancement, inversion) and simple transformation (cropping, projection). These features were intended to make image post-processing easier/faster with the limited microprocessors of the mid 90s. The analog output then has to be converted into a digital signal, one pixel after the other in series, to be processed further, which represents the major bottleneck in terms of data flow rate (plus the non negligible additional cost of a fast analog to digital converter or ADC). The Game Boy Camera deals with this shortcoming by incorporating a flash ADC in its mapper but is capped to about 12 fps nevertheless (which is a _tour de force_ by itself considering the obsolete technology involved in the Game Boy, even by 1998 standard). Enough for a toy, probably not for real time applications. Sensors by themselves could _theoretically_ spit 30 fps at 500 kHz in 128x128 pixels if the ADC converting pixels and embedded memory would be fast enough. It was not the case. The pure analog output more or less quickly relegated these sensors to the status of digital history curiosity.

To overcome this weakness, the sensors in the artificial retina series evolved towards random tile addressing and projection to reduce the flow rate of pixels to convert (M64283FP) and lower definition (the M64285FP with 32x32 pixels matrix, is _theoretically_ able to output 5000 fps), but eventually disappeared in favor of faster, fully digital and color sensors. It is **totally undocumented** to this date which exact artificial retina equipped with exact apparatus and what the production volume was, except for the Game Boy Camera that reaches a world recognition. The M64282FP itself being probably Nintendo Exclusive, it is safe to assume that any alledged 128x128 pixels artificial retina used in a _non-Game Boy Camera device_ was a M64283FP. Safe to assume but not a proof in any case without disassembly.
...
The M64283FP was anyway probably available for world retail at a certain point as it had a short carreer in labs around the 2000s to tinker augmented vision systems, both in Japanese laboratory and in the West. But they were yet in competition with faster full digital sensors.

This sensor appears sporadically on Japanese online auction sites for random prices (from "given for a complete tray in its original packaging" to "total scam for one loose unit") and some Chinese chip dealers claim to have some, but I've never managed to bargain one for less than 60€ (so typically high on the scam side) nor to get a single proof that they have this sensor in stock for real (I mean at least a single image not stolen from elsewhere for example...). I finally obtained three units of the M64283FP in 2023 from a generous donator who know someone who knew someone at Mitsubishi, knowing that these sensors will have a good second home with me. In brief, to get one, be patient (and lucky).

Early CMOS based sensors have some particular "image signatures". The use of a fix amplifier assigned to each pixel creates images with a fixed-pattern noise, they cannot simultaneously charge all the pixels of an image at once, which leads to image distorsion in the direction of movement (rolling shutter effect) and their dynamic range is rather poor. The images produced by the M64283FP have all of these signatures.
